...
class Ship:
	...
	self.start = start
	self.end = end
	self.distance = self.calcDistance("ship",start,end)

    # 一般化費用
    def calcGeneralCost(self,a,b,c,start,end,price,totalweight):
    	try:
		result =  a*self.calcFee("ship",start,end,totalweight)*self.traffic(totalweight) + b*self.calcTime("ship",start,end)*self.calcTimeValue(price) + c*self.calcELoad("ship",start,end,totalweight)*self.calcDistance("ship",start,end)*self.UnitCost("ship",start,end)*self.traffic(totalweight)
		if result == 0:
			return 0
		else:
			return result
