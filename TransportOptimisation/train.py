...
class Train:
	...
	self.start = start
	self.end = end
	self.distance = self.calcDistance("train",start,end)

        	        
    # 一般化費用
    def calcGeneralCost(self,a,b,c,start,end,price,totalweight):
	try:
		result =  a*self.calcFee("train",start,end,totalweight)*self.traffic(totalweight) + b*self.calcTime("train",start,end)*self.calcTimeValue(price) + c*self.calcELoad("train",start,end,totalweight)*self.calcDistance("train",start,end)*self.UnitCost("train",start,end)*self.traffic(totalweight)
		if result == 0:
			return 0
		else:
			return result
